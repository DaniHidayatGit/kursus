<?php
    ...
    $sql_data_pengajar = "SELECT * FROM pengajar WHERE kode_pengajar = ? ";
    $check_pengajar = $con->prepare($sql_data_pengajar);
    $check_pengajar->bind_param('i', $kode_pengajar);
    $check_pengajar->execute();            
    $check_pengajar->store_result();
    $check_pengajar->bind_result($kode_pengajar, $nama_pengajar, $nik, $alamat,$nomor_telepon, $jenis_kelamin);
    $row = array();
    if ($check_pengajar->fetch()) {
        $row = array('kode_pengajar'=>$kode_pengajar, 'nama_pengajar'=>$nama_pengajar, 'nik'=>$nik, 'alamat'=>$alamat, 'nomor_telepon'=>$nomor_telepon, 'jenis_kelamin'=>$jenis_kelamin);
    }
                ...
                <div class="row">
                    <div class="col-xl-12 col-lg-7">
                        <div class="card shadow mb-5">
                            <form class="user col-md-auto" method="POST" action="cek_edit_pengajar.php">
                                <br>
                                    <div class="col-xl-5 col-lg-7">
                                        <i> Kode pengajar : </i>
                                        <input type="text" value="<?= $row['kode_pengajar']; ?>" placeholder="Kode Pengajar" name="kode_pengajar" class="form-control form-control-user" required readonly>
                                    </div>
                                <br>  
                                    <div class="col-xl-5 col-lg-7">
                                        <i> Nama Pengajar : </i>
                                        <input type="text" value="<?= $row['nama_pengajar']; ?>" placeholder="Nama Pengajar" name="nama_pengajar" class="form-control form-control-user" pattern="[a-z]{3,25}" required>
                                    </div> 
                                <br>
                                    <div class="col-xl-5 col-lg-7">
                                        <i> NIK : </i>
                                        <input type="text" value="<?= $row['nik']; ?>" placeholder="NIK" autocomplete="on" name="nik" class="form-control form-control-user" pattern="[0-9]{16,16}" required readonly>
                                    </div> 
                                <br>
                                    <div class="col-xl-5 col-lg-7">
                                        <i> Alamat : </i>
                                        <input type="text" value="<?= $row['alamat']; ?>" placeholder="Alamat" class="form-control form-control-user" name="alamat" required>
                                    </div>
                                <br>
                                    <div class="col-xl-5 col-lg-7">
                                        <i> Nomor Telepon : </i>
                                ...
                                <br>
                                    <input type="submit" value="Ubah" class="btn btn-success btn-block">
                                <br>
                            </form>
                        </div>
                    </div>
