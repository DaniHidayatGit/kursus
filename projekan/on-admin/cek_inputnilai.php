<?php
    ...
    $sql_cek = "SELECT nama_mata_pelajaran FROM mata_pelajaran WHERE kode_mata_pelajaran = ?";
    $check_cek = $con->prepare($sql_cek);
    $check_cek->bind_param('s', $mapel);
    $check_cek->execute();
    $check_cek->store_result();
    $check_cek->bind_result($nama_mata_pelajaran);
    $row = array();

    $sql_cek1 = "SELECT nama_pengajar FROM pengajar WHERE kode_pengajar = ?";
    $check_cek1 = $con->prepare($sql_cek1);
    $check_cek1->bind_param('i', $pengajar);
    $check_cek1->execute();
    $check_cek1->store_result();
    $check_cek1->bind_result($nama_pengajar);
    $rows = array();

    if ($check_cek->fetch()) {
        $row = array('nama_mata_pelajaran'=>$nama_mata_pelajaran);
    }
    if ($check_cek1->fetch()) {
        $rows = array('nama_pengajar'=>$nama_pengajar);
    }

    if ($nilai >= 0 && $nilai <= 100 && $nilai != NULL) {
        $sql_update = "UPDATE data_nilai SET nama_mata_pelajaran = ?, kode_mata_pelajaran = ?, nilai = ?, kode_pengajar = ?, nama_pengajar = ? WHERE username = ?";
        $check_update = $con->prepare($sql_update);
        $check_update->bind_param('siiiss', $row['nama_mata_pelajaran'], $mapel, $nilai, $pengajar, $rows['nama_pengajar'], $username);
...
<head>
    ...
    <title>E-KURSUS - Input Nilai </title>
            ...
            <li class="nav-item active">
                ...
                <div id="collapsePages" class="collapse" aria-labelledby="headingPages" data-parent="#accordionSidebar">
                    <div class="bg-white py-2 collapse-inner rounded">
                        ...
                        <a class="collapse-item" href="flowchart.php"> Flow Chart Aplikasi </a>
                    </div>
                </div>
            </li>
                    ...
                    <div class="col-xl-12 col-lg-7">
                        <div class="card shadow mb-5">
                            <div class="col-md-auto">
                                ...
                                <br>
                                    <a href="index.php" class="btn btn-success btn-block"> Kembali Ke Menu </a>
                                <br>
                            </div>
                        </div>
                    </div>
