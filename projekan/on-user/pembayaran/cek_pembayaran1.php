<?php
    ...
    $nama_username = $_SESSION['nama'];
    
    if ($tipe_file = "jpg" || $tipe_file = "png" || $tipe_file = "jpeg") {
        if ($ukuran_file <= 8132210) {
            move_uploaded_file($tmp_file, $path);
            $sql_data = "UPDATE data_user SET bukti_pembayaran = ?, valid = ?, paket = ? WHERE username = ? ";
            $check_data = $con->prepare($sql_data);
            $check_data->bind_param('ssis', $nama_file, $test ,$paket, $nama_username);
